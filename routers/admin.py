...
# Make sure to import your configured Supabase client and Auth dependency
...
def recalculate_all_user_balances():
    try:
        ...
        for prof in all_profiles:
            uid = prof["id"]
            u_bets = supabase.table("user_bets").select("week_number, wager_amount, pick, weekly_questions(winning_answer)").eq("user_id", uid).execute().data
            u_td = supabase.table("touchdown_picks").select("week_number, is_correct").eq("user_id", uid).eq("is_correct", True).execute().data
...
async def view_admin_panel(
    ...
    # Fetch Real User Profile for Sticky Bar
    ...
    # Fetch active weeks to deduct pending wagers
    ...
    # Week Options Logic (Include capability to add next week)
...
async def publish_questions(request: Request, week_number: int = Form(...)):
    ...
    return RedirectResponse(url=f"/admin?manage_week={week_number}&active_tab=questions&msg=Week+{week_number}+Published+Successfully", status_code=303)
...
async def clear_week(week_number: int = Form(...)):
    ...
    return RedirectResponse(url=f"/admin?manage_week={week_number}&active_tab=questions&msg=Week+{week_number}+Cleared", status_code=303)
