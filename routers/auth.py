...
async def signup_user(
    ...
    try:
        ...
        if create_res and create_res.user:
            ...
            # Robust token extraction handling dictionary or object attributes safely
            token_to_use = None
            if link_response:
                props = getattr(link_response, "properties", None) or (link_response.get("properties") if isinstance(link_response, dict) else None)
                if props:
                ...
                # Fallback check directly on response attributes if properties structure differs
                if not token_to_use:
                    token_to_use = getattr(link_response, "hashed_token", None) or getattr(link_response, "token_hash", None)

            if token_to_use:
                verification_link = f"https://tdtokens.co.uk/auth/verify?token={token_to_use}&type=signup"
...
async def request_password_reset(
    ...
    try:
        ...
        token_to_use = None
        if response:
            props = getattr(response, "properties", None) or (response.get("properties") if isinstance(response, dict) else None)
            if props:
                hashed_token = props.get("hashed_token") if isinstance(props, dict) else getattr(props, "hashed_token", None)
                email_otp = props.get("email_otp") if isinstance(props, dict) else getattr(props, "email_otp", None)
                token_to_use = hashed_token or email_otp
            if not token_to_use:
                token_to_use = getattr(response, "hashed_token", None) or getattr(response, "token_hash", None)
        
        recovery_link = f"https://tdtokens.co.uk/auth/login?token={token_to_use}&type=recovery" if token_to_use else "https://tdtokens.co.uk/auth/login"
