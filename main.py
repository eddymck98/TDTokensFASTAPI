...
app.include_router(profile.router, prefix="/profile", tags=["Profile Management"]) if hasattr(profile.router, "router") else app.include_router(profile.router, prefix="/profile", tags=["Profile Management"])
...
@app.get("/dashboard/", response_class=HTMLResponse)
async def dashboard_endpoint_slash(request: Request):
    return await dashboard.render_dashboard_or_index(request)
...
@app.get("/rules/", response_class=HTMLResponse)
async def rules_page_slash(request: Request):
    return await rules_page(request)
...
@app.get("/history/", response_class=HTMLResponse)
async def history_page_slash(request: Request):
...
@app.get("/settings/", response_class=HTMLResponse)
async def settings_page_slash(request: Request):
    return RedirectResponse(url="/profile/", status_code=303)
